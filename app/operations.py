...
async def payment_by_user_id(operation: schemas.OperationPaymentCreate, db: Session = Depends(get_db)):
    db_user = await crud_utils.get_user_by_id(db, user_id=operation.id_user)
    ...
    new_balance = await crud_utils.create_operation_payment(db, operation, 'payment')
    if new_balance < 0:
...
async def replenishment_by_user_id(operation: schemas.OperationReplenishmentCreate, db: Session = Depends(get_db)):
    db_user = await crud_utils.get_user_by_id(db, user_id=operation.id_user)
    if db_user is None:
        raise HTTPException(status_code=404, detail=f"User with id=\'{operation.id_user}\' not found")
    new_balance = await crud_utils.create_operation_replenishment(db, operation, 'replenishment')
